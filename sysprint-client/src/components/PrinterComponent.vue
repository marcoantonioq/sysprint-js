<template>
  <div ref="printerHTML" v-if="props.printer">
    <input class="hidden" type="file" accept=".pdf" multiple @change="handleFileChange" />
    <q-dialog v-model="dialog" position="top">
      <q-card class="card">
        <q-card-section>
          <div class="text-center q-mb-sm" style="color: #4b4b4b">
            <q-icon name="print" style="font-size: 20px; margin-right: 4px" />
            <b> {{ props.printer.name.toUpperCase().replace(/(_|-)/gi, ' ') }}</b>
          </div>

          <div class="row justify-center actions">
            <div class="text-center">
              <q-btn color="primary" size="md" @click="openFileInput" style="height: 77px" class="upload">
            ...
            <div class="column">
              <q-btn color="green-9" label="Imprimir" @click="printer()" style="margin: 0 5px 5px"
                :disabled="data.files.length === 0" />

              <q-btn class="btn-cancel" color="red-8" label="Cancelar" @click="cancel" style="margin: 0 5px" />
            </div>
          </div>
          <q-input type="number" v-model="data.spool.copies" label="Cópias" />
          <q-input id="range" type="text" v-model="data.spool.range"
            @change="data.spool.range = data.spool.range?.replace(/[^0-9,-]/gi, '').trim()" class="validate"
            label="Páginas (ex: 1-5 ou 2,4)" />

          <div class="text-left">
            <div class="label">Folhas:</div>
            <q-radio v-model="data.spool.pages" val="all" label="Todas" />
            <q-radio v-model="data.spool.pages" val="even" label="Folhas pares" />
            <q-radio v-model="data.spool.pages" val="odd" label="Folhas impares" />
          </div>
          ...
          <div class="text-left">
            ...
            <q-radio v-model="data.spool.quality" val="3" label="Rascunho" />
            <q-radio v-model="data.spool.quality" val="4" label="Normal" />
            <q-radio v-model="data.spool.quality" val="5" label="Melhor" />
          </div>
          ...
          <div class="text-left">
            ...
            <q-radio v-model="data.spool.media" :val="undefined" label="Automático" />
            <q-radio v-model="data.spool.media" val="A4" label="A4" />
            <q-radio v-model="data.spool.media" val="A3" label="A3" />
          </div>
        </q-card-section>
      </q-card>
    </q-dialog>
...
const printerHTML = ref(null)
const props = defineProps({ printer: Object })
...
const printer = () => {
  ...
  if (props.printer) {
    const jobs = data.files.map((file) => {
      const job = JSON.parse(JSON.stringify(data.spool))
...
const openFileInput = () => {
  const fileInput = printerHTML.value?.querySelector('input[type="file"]')
  if (fileInput) {
    fileInput.click()
  }
}
